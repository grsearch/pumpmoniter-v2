...
class HeliusMonitor {
  ...
  _handleMessage(msg) {
    ...
    if (msg.method === 'logsNotification') {
      ...
      if (!this._isPumpMigrationLogs(logs)) return;

      console.log(`[Helius] WS migration log [${SOURCE_PUMP}]: ${sig?.slice(0, 8)}...`);
  ...
  // ──────────────────────────────────────────────
  // Polling fallback（轮询 pump 迁移钱包）
  // ──────────────────────────────────────────────
  _startPolling() {
    this._pollInit().then(() => {
      this.pollTimer = setInterval(() => this._pollOnce(), POLL_INTERVAL_MS);
      console.log(`[Helius] Polling started (every ${POLL_INTERVAL_MS / 1000}s) — pump wallet`);
    }).catch(err => {
  ...
  async _pollInit() {
    const pumpSigs = await this._fetchSigs(PUMP_MIGRATION_WALLET);
    pumpSigs.forEach(s => this.seenSigs.add(s.signature));
    console.log(`[Helius] Polling baseline: ${this.seenSigs.size} sigs (pump=${pumpSigs.length})`);
  }

  async _pollOnce() {
    try {
      await this._processNewSigs(PUMP_MIGRATION_WALLET);
  ...
  async _parseTxFromRpc(signature) {
    ...
    const mint = this._extractPumpMint(tx);

    if (!mint) {
      const post = tx.meta?.postTokenBalances || [];
      console.log(`[Helius][${SOURCE_PUMP}] mint extract failed sig=${signature.slice(0, 8)} mints=${JSON.stringify(post.map(b => b.mint))}`);
      return;
    }
    ...
    console.log(`[Helius] ✅ [${SOURCE_PUMP}] Migration: mint=${mint} dev=${devAddress || 'unknown'}`);
  ...
  _emit(mint, signature, devAddress) {
    const event = { mint, signature, devAddress, source: SOURCE_PUMP, symbol: null, name: null };
